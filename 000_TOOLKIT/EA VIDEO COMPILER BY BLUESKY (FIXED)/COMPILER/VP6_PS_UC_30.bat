@echo off
setlocal enabledelayedexpansion

:: Make the workload.
if not exist ".\temp" mkdir "temp"
if not exist ".\temp\avi" mkdir "temp\avi"
if not exist ".\temp\avi\avi_vp6" mkdir "temp\avi\avi_vp6"
if not exist ".\temp\avi\avi_mpeg4" mkdir "temp\avi\avi_mpeg4"
if not exist ".\temp\Wav" mkdir "temp\Wav"
if not exist ".\temp\Final" mkdir "temp\Final"
if not exist ".\temp\Final" mkdir "Release"
for %%f in (MP4\*.mp4) do (mkdir "temp\Final\%%~nf")

@echo off
for %%f in (mp4\*.mp4) do (
    set "filename=%%~nf"
    tools\FFMPEG\ffmpeg -i "%%f" -c:v libxvid -r 30 -b:v 17M -an ".\temp\avi\avi_mpeg4\!filename!.avi"
)

:: rem Convert MP4 to WAV using ffmpeg
for %%f in (mp4\*.mp4) do (
    set "filename=%%~nf"
    tools\FFMPEG\ffmpeg -i "%%f" -acodec pcm_s16le -ar 44100 -ac 2 ".\temp\Wav\!filename!.wav"
)


for %%f in (MP4\*.mp4) do (mkdir "temp\Final\%%~nf")

:: rem Convert WAV to ASF using sx_2004
for %%f in (temp\WAV\*.wav) do (
  tools\sx\sx -sndstream -eaxa_blk -fps30 "temp\WAV\%%~nf.wav" -= ".\temp\final\%%~nf\%%~nf.asf"
)

:: Convert AVI to VP6 using your specified command
for %%f in (temp\avi\avi_mpeg4\*.avi) do (
    set "filename=%%~nf"
    tools\VP6\gx -avi -config -pack -v4 "%%f"=".\temp\avi\avi_vp6\!filename!.avi"
)

:: Fix the header of vp6
for %%f in (temp\avi\avi_vp6\*.avi) do (
    set "filename=%%~nf"
    tools\VP6\VP6HeaderFix.exe "%%f"
)
:: Convert AVI to VP6 using nihav-encoder and dump to folder
for %%f in (temp\avi\avi_vp6\*.avi) do (
    set "filename=%%~nf"
    tools\VP6\nihav-encoder --input "%%f" --output ".\temp\final\!filename!\!filename!.vp6" --output-format ea --verbose)


for %%f in (mp4\*.mp4) do (
    set "filename=%%~nf"
    tools\quickbms\quickbms.exe -o tools\quickbms\eavp6_muxer.bms .\temp\final\!filename!\ .\release
)

:: rem Remove .asf from filenames
for %%f in (Release\*.vp6) do (
    set "filename=%%~nf"
    set "newfilename=!filename:.ASF.vp6=!"
    ren "%%f" "!newfilename!.vp6"
)

:: rem Remove .ASF from filenames
for %%f in (Release\*.ASF*) do (
    set "filename=%%~nf"
    set "extension=%%~xf"
    set "newfilename=!filename:.ASF=!%%~xf"
    ren "%%f" "!newfilename!"
)

:: rem Remove .ASF from filenames
for %%f in (Release\*.vp6) do (
    set "filename=%%~nf"
    "Tools\PS_UC\VP6SHEN.exe" "%%f"
)

:: Deletes temp when compile is done.
rmdir /s /q temp

endlocal


